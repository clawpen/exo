...
use std::io::Read;
use std::fs::File;
use exo_image::{ImageStore, ImageReference};
...
pub async fn execute(args: ImportArgs) -> Result<()> {
    ...
    let manifest: Vec<serde_json::Value> = serde_json::from_slice(&manifest_data)
        .context("Failed to parse manifest.json")?;
    
    let first_image = manifest.first()
    ...
    // Read config
    let config_file = first_image.get("Config")
        .and_then(|c| c.as_str())
    ...
    // Extract layers
    let layer_digests = first_image.get("Layers")
    ...
    for (i, layer_path) in layer_digests.iter().enumerate() {
        ...
        println!("    Layer {}/{}: {}", i + 1, layer_digests.len(), layer_path);
        ...
        if is_gzipped {
            ...
            layer_archive.unpack(&rootfs_path)
