...
impl Syscall {
    ...
    fn as_scmp_syscall(&self) -> ScmpSyscall {
        match self {
            Syscall::Name(name) => ScmpSyscall::from_name(name).unwrap_or_else(|_| {
                ScmpSyscall::from_name("invalid").unwrap_or(ScmpSyscall::from(-1))
            }),
...
pub fn default_profile() -> SeccompProfile {
    ...
    let essential = vec![
        ...
        "pause",
        "fcntl",
        "fcntl64", // File control - needed by Node.js
        "nanosleep",
        "getitimer",
        "alarm",
        "setitimer",
...
pub fn apply_seccomp(profile: &SeccompProfile) -> Result<()> {
    ...
    for rule in &profile.arg_rules {
        ...
        let arg_cmp = match compare {
            SeccompCompare::Eq(val) => {
                ScmpArgCompare::new(rule.arg_num as u32, ScmpCompareOp::Equal, val)
            }
            SeccompCompare::NotEq(val) => {
                ScmpArgCompare::new(rule.arg_num as u32, ScmpCompareOp::NotEqual, val)
            }
            SeccompCompare::Gt(val) => {
                ScmpArgCompare::new(rule.arg_num as u32, ScmpCompareOp::Greater, val)
            }
            SeccompCompare::Ge(val) => {
                ScmpArgCompare::new(rule.arg_num as u32, ScmpCompareOp::GreaterEqual, val)
            }
            SeccompCompare::Lt(val) => {
                ScmpArgCompare::new(rule.arg_num as u32, ScmpCompareOp::Less, val)
            }
            SeccompCompare::Le(val) => {
                ScmpArgCompare::new(rule.arg_num as u32, ScmpCompareOp::LessOrEqual, val)
            }
...
mod tests {
    ...
    fn test_dangerous_syscalls() {
        ...
        assert!(dangerous
            .iter()
            .any(|s| matches!(s, Syscall::Name(n) if n == "reboot")));
        assert!(dangerous
            .iter()
            .any(|s| matches!(s, Syscall::Name(n) if n == "init_module")));
    }
