...
pub fn setup_overlay_rootfs(container_id: &str) -> Result<PathBuf> {
    ...
    // Both failed - fall back to a read-only rootfs using the first lowerdir.
    let fallback = paths
        .lower
        .first()
        .cloned()
    ...
    tracing::warn!(
        "Both kernel overlay and fuse-overlayfs failed; using read-only rootfs at {:?}",
        fallback
    );
    Ok(fallback)
}
...
/// Inject container-specific network files into the rootfs.
///
/// Should be called after pivot_root, inside the container's mount namespace,
/// for `bridge` and `none` network modes.
#[cfg(target_os = "linux")]
pub fn inject_network_files(
    hostname: &str,
    container_ip: Option<&str>,
    dns_servers: &[&str],
    hosts_entries: &[(String, String)],
) -> Result<()> {
    // /etc/resolv.conf
    ...
    resolv_content.push_str("# Generated by exo\n");
    if dns_servers.is_empty() {
        resolv_content.push_str("nameserver 8.8.8.8\n");
        resolv_content.push_str("nameserver 8.8.4.4\n");
    } else {
        for dns in dns_servers {
            resolv_content.push_str(&format!("nameserver {}\n", dns));
        }
    }
    std::fs::write(resolv, resolv_content)
        .with_context(|| "Failed to write /etc/resolv.conf")?;
    ...
    hosts_content.push_str("# Generated by exo\n");
    hosts_content.push_str("127.0.0.1\tlocalhost\n");
    hosts_content.push_str("::1\tlocalhost ip6-localhost ip6-loopback\n");
    ...
    std::fs::write(hosts, hosts_content)
        .with_context(|| "Failed to write /etc/hosts")?;
